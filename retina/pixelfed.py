"""Fetch images from a Pixelfed account's media timeline, with cursor-based resume.

Pixelfed implements the Mastodon-compatible /api/v1/accounts/* endpoints, so
the shared _mastoapi helpers drive all the logic. Account handles follow the
same @user@instance format as Mastodon, e.g. @art@pixelfed.social.
"""
...
def _filename_for_item(account_handle: str, image_url: str) -> str:
    """Deterministic filename for a Pixelfed image (pixelfed_<slug>_<hash><ext>)."""
    return _shared_filename("pixelfed", account_handle, image_url)
